...
import resnext.resnext as resnext
...
def main():
    ...
    parser.add_argument('--resnet_root', default='./resnext', help='path the pre-trained cnn models')
    parser.add_argument('--crop_size', type=int, default=224, help='crop size of image')
    parser.add_argument('--path_image', default='../pytorch-pretrained-BERT/twitter_subimages/', help='path to images')
    parser.add_argument('--mm_model', default='TomBert',
                        help='model name')  # TomBert, TomBertNoPooling, MBert, MBertNoPooling, ResBert
    ...
    if args.mm_model == 'ResBert':
        ...
        model = TomBertForMMSequenceClassification.from_pretrained(args.bert_model,
                                                                   ...
                                                                   num_labels=num_labels,
                                                                   pooling=args.pooling)
    net = getattr(resnext, 'resnext101_32x16d_wsl')()
    model_resnext = torch.hub.load('facebookresearch/WSL-Images', 'resnext101_32x16d_wsl')
    net.load_state_dict(model_resnext)
    encoder = myResnext(net, args.fine_tune_cnn, device)
    if args.fp16:
        model.half()
        encoder.half()
    model.to(device)
